...
print('ETL de paineis de energia solar dos EUA (net-metering)')
# 2. Baixando arquivos
...
for item in soup.find_all("a"):
    if item.get('href') and item.get('title'):
        if 'Net metering' in item.get('title'):
            list_url_download.append((url+item['href']).replace('./',''))


list_url_download

folder_path = f'C:\\Users\\{user}\\ATMOSPHERE\\Atmosphere Capital - Capital - Market Inteligence\\Data Analysis\\etl_capacidade_solar\\etl_eia_net_metering'
...
for file_i in files_paths:
    ...
    for sheet_name_j in sheet_names_i:
        if "utility" in sheet_name_j.lower():
            df_i = pd.read_excel(excel_file_i , sheet_name= sheet_name_j, header = [0,1,2], skiprows=1)
            break
    header_concat_i = []
    for header0,header1,header2 in df_i.columns:
        if "unnamed:" in str(header1).lower():
            header_concat_j = header0 +"_id_"+str(header2)
        else:
            header_concat_j = str(header0) + "_" + str(header1) +"_" +str(header2)
...
for df_i in list_df:
    ...
    for col in df_i.columns:
        new_columns.append(col.replace('charateristics','characteristics'))
    df_i.columns = new_columns
...
for col in df.columns:
    try:
        col_split_1 , col_split_2 , col_split_3 = col.split("_")
        new_name = col_split_2 + "_" + col_split_3
    except:
        new_name = col
...
df = df[(df['variable'].str.contains('photovoltaic_capacity')) |
   (df['variable'].str.contains('pv-pairedbatterycapacity')) |
   (df['variable'].str.contains('photovoltaic_storagecapacity'))]

# 4. Importando para o banco de dados
...
df['id_state'] = df['id_state'].astype(str)
...
eia_netmetering_collection = mdb.client['gestao']['energy.eia_netmet_sol_capac']
mongo.bulk_update(eia_netmetering_collection,dict_list)
mdb.client.close()
